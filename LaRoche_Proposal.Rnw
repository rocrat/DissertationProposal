...
%
This difference plays an important role in principal components analysis (PCA) and the measurement of distance.  With respect to PCA, Aitchison (1983)~\cite{Aitchison1983} identifies 3 important properties of a transformation which make it suitable for principle components analysis: 1) it must have an interpretable covariance structure, 2) it must be symmetric in its treatment of the components (invariant to permutations), and 3) it must deal with the inherent curvature of compositions.  Both the CLR and the CPM address the curvature problem to some degree through the use of the log transformation.%Not really sure about this
The CLR transformation additionally removes the sum constraint which complicates the interpreation of the covariance matrix (and the resulting eigenvalues and eigenvectors) while preserving the symmetry of treatment of the components. \\
