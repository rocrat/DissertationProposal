...
Ribonucleic acid (RNA) has become a major target of investigation for a wide variety of research areas in biology due to its role in the function of cells, including gene transcription and regulation.  Measurements of RNA in biological samples is increasingly performed using RNA-Seq \cite{Wang2009} on high-thoughput next generation sequencing (NGS) platforms.  RNA-Seq offers several advantages over a traditional micro-array experiment and has been rapidly adopted by scientists \cite{Illumina}.  However, the analysis of RNA-Seq data is complicated by the relative frequency nature of data produced by NGS platforms.  The current research is restricted to the analysis of extraction-free targeted RNA-Seq data and the unique analytical challenges it creates.
...
\subsubsection{Compositional Invariance}
\label{sec:compInv}

Up to this point I have assumed that the size of a composition, i.e. the sum of the components, contains no information.  Aitchison (1986)~\cite{Aitchison1986} refers to a raw composition as a basis and points out that for any given composition, $\mathbf{x}: \sum \mathbf{x}=1$ there are an uncountable number of bases which can be acheived by scaling the composition by a constant; 
...
Aitchison defines compositonal invariance as the statistical independence of $\mathbf{x}$ and $t$.  He then defines a test for compositional invariance based on the covariance between the composition formed from the basis through the closure operation and the basis size, $t$.  This is accomplished through the linear model, 

\begin{equation}
\mathbf{Y = A \Theta + E}
\label{compInvariance}
\end{equation}

where $\mathbf{Y}$ is the ALR transformed composition array,  $\mathbf{A}$ is an $N \times 2$ matrix with $r$th row $\left[\text{1 log } t_r \right]$, and 
...
The statistical hypothesis of compositional invariance is that $\beta_1 = \cdots = \beta_d = 0$\\

If the data satisfy the assumption of logistic normality then hypothesis testing can be acheived%...
...
\begin{align}
CPM^*(x_i) &= log\left(\frac{x_i}{\sum \mathbf{x} } \right)\\
...
%
This difference plays an important role in principal components analysis (PCA) and the measurement of distance.  With respect to PCA, Aitchison (1983)~\cite{Aitchison1983} identifies 3 important properties of a transformation which make it suitable for principle components analysis: 1) it must have an interpretable covariance structure, 2) it must be symmetric in its treatment of the components (invariant to permutations), and 3) it must deal with the inherent curvature of compositions.   \\

The inherent curavature of compositions leads to problems with measuring distance.  Specifically, the distance metric between any two compositions must be invariant to changes of location.  In the simplex, changes in location are acheived through the perturbation operator which is the compositional equivalent of translation in euclidean space (see~\ref{subsubsec:perturbation}).  Therefore, any distance metric must be invariant to equal changes in location of the two compositions.

\begin{mydef}
Perturbation invariance for a distance metric, $D$, is defined as;
\begin{equation}
D(q\odot x, q \odot y) = D(x, y),
\label{pertInv}
\end{equation}
...
This property is analogous to translation invariance for Euclidean distance metrics.  The CLR transformation results in perturbation invariance when used with standard Euclidean distance. Let $\mathbf{q}$ be a $D$-dimensional perturbation.  Additionally, let $\mathbf{x}$ and $\mathbf{y}$ represent two $D$-dimensional compositions.  Applying the Euclidean distance metric to CLR transformed x and y we have,

\begin{align}
D(x, y) &= \left[ \sum_i^D \left(  log \left(\frac{x_i}{g(\mathbf{x})} \right) - log\left(\frac{y_i}{g(\mathbf{y})} \right) \right)^2 \right]^{1/2}\\
  &= \left[ \sum_i^D \left( log \left(\frac{x_i}{y_i} \right) - log(g(\mathbf{x})) + log(g(\mathbf{y})) \right)^2 \right]^{1/2}\\
  &= \left[ \sum_i^D \left( log \left(\frac{x_i}{y_i} \right) - \frac{\sum_i^D log \mathbf{x}}{D} + \frac{\sum_i^D log \mathbf{y}}{D} \right)^2 \right]^{1/2}
  \label{CLRdist}
\end{align}

Set $\mathbf{u} = \mathbf{q} \odot \mathbf{x}$ and $\mathbf{v} = \mathbf{q} \odot \mathbf{y}$.  Equation~\ref{CLRdist} then becomes,

\begin{align}
D(u, v) &= \left[ \sum_i^D \left( log \left(\frac{u_i}{v_i} \right) - \frac{\sum log \mathbf{u}}{D} + \frac{\sum log \mathbf{v}}{D} \right)^2 \right]^{1/2}\\
  &= \left[ \sum_i^D \left( log \left(\frac{q_ix_i}{q_iy_i} \right) - \frac{\sum_i^D log q_ix_i}{D} + \frac{\sum_i^D log q_iy_i}{D} \right)^2 \right]^{1/2}\\
  &= \left[ \sum_i^D \left( log \left(\frac{q_ix_i}{q_iy_i} \right) - \frac{\sum_i^D log q_i + \sum_i^D log x_i}{D} + \frac{\sum_i^D log q_i + \sum_i^D y_i}{D} \right)^2 \right]^{1/2}\\
  &= \left[ \sum_i^D \left( log \left(\frac{x_i}{y_i} \right) - \frac{\sum_i^D log q_i}{D} - \frac{\sum_i^D log x_i}{D} + \frac{\sum_i^D log q_i}{D} + \frac{\sum_i^D y_i}{D} \right)^2 \right]^{1/2}\\
  &= \left[ \sum_i^D \left( log \left(\frac{x_i}{y_i} \right) - \frac{\sum_i^D log \mathbf{x}}{D} + \frac{\sum_i^D log \mathbf{y}}{D} \right)^2 \right]^{1/2}\\
  &= D(x, y)
\label{CLRpertDist}
\end{align}
...
\begin{align}
D(x, y) &= \left[ \sum_{i=1}^D \left( log\left( \frac{x_i}{\sum \mathbf{x}} \right) - log\left( \frac{y_i}{\sum \mathbf{y}} \right) \right)^2 \right]^{1/2}\\
  &= \left[ \sum_{i=1}^D \left( log(x_i) - log(y_i) - log(\sum \mathbf{x}) + log(\sum \mathbf{y}) \right)^2 \right]^{1/2}\\
  &= \left[ \sum_{i=1}^D \left( log\left( \frac{x_i}{y_i} \right) + log\left( \frac{\sum \mathbf{x}}{\sum \mathbf{y}} \right) \right)^2 \right]^{1/2}
\label{CPMDist}
\end{align}

Again, let $\mathbf{u} = \mathbf{q} \odot \mathbf{x}$ and $\mathbf{v} = \mathbf{q} \odot \mathbf{y}$.  Equation~\ref{CPMDist} then becomes,

\begin{align}
D(u, v) &= \left[ \sum_{i=1}^D \left( log\left( \frac{u_i}{v_i} \right) + log\left( \frac{\sum \mathbf{u}}{\sum \mathbf{v}} \right) \right)^2 \right]^{1/2}\\
  &= \left[ \sum_{i=1}^D \left( log\left( \frac{q_ix_i}{q_iy_i} \right) + log\left( \frac{\sum_{i = 1}^D q_iy_i}{\sum_{i = 1}^D q_ix_i} \right) \right)^2 \right]^{1/2}\\
  &= \left[ \sum_{i=1}^D \left( log\left( \frac{x_i}{y_i} \right) + log\left( \frac{\sum_{i = 1}^D q_iy_i}{\sum_{i = 1}^D q_ix_i} \right) \right)^2 \right]^{1/2}\\
  &= D(x, y) \iff q_i = k ~\forall~ i \in 1 \dots D,
\label{CPMpertDist}
\end{align}
